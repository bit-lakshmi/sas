...
import os, time, mysql.connector, cgi, datetime
...
cursor = cnx.cursor()
sql = "SELECT pdisk_name, pname FROM projects WHERE pcode = '{}'".format(SCODE)
cursor.execute(sql)
...
for shot in syncshots:
    ...
    print "<br>File Name: <a id='syncpath' data-host='"+HOST+"' data-pdiskn='"+str(shot[6])+"' style='cursor: pointer;'>" + str(shot[4]) + "</a>"
    print("<br>Last modified: %s" % str(shot[7]))
    print("<br>Created: %s" % str(shot[9]))
    print "<p></div><div class=\"card-action\"><a id='syncpath' data-host='{}' data-pdiskn='{}' style='cursor: pointer;'>Explorer</a></div></div>".format(HOST,str(shot[6]))
cnx.close()
s3db.close()
